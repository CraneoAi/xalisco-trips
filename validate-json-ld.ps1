...
$content = Get-Content "index-en.html" -Raw
$pattern = '(?s)<script type="application/ld\+json">(.*?)</script>'
$matches = [regex]::Matches($content, $pattern)

Write-Host "Validating $($matches.Count) JSON-LD blocks...`n" -ForegroundColor Cyan
...
for ($i = 0; $i -lt $matches.Count; $i++) {
    ...
    try {
        ...
        Write-Host "Content: $($json.Substring(0, [Math]::Min(200, $json.Length)))..." -ForegroundColor Gray
    }
}
